    ...
    for i in tqdm(range(0, len(messages), args.batch_size)):
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            for id_idx in range(len(batch_output_text)):
                bboxes = [] # Initialize bboxes to ensure it exists for the error block
                try:
                    ...
                    all_outputs.append({
                        ...
                        "union": int(union),
                        "bbox_iou": bbox_iou,
                        "pred_bboxes": [],             # Added empty list for failed extraction
                        "gt_bbox": batch_id_list[id_idx].get("bbox", []) # Added ground truth bbox
                    })
                    continue
                try:
                ...
                if has_bbox:
                    try:     
                        ...
                        # --- START MODIFICATION ---
                        # Instead of checking for > 0.5, find the best IoU
                        best_iou_for_image = 0.0 
                        for pred_bbox in bboxes:
                            current_iou = compute_bbox_iou(pred_bbox, gt_bbox)
                            if current_iou > best_iou_for_image:
                                best_iou_for_image = current_iou
                        
                        # Save the actual float value (e.g., 0.874)
                        bbox_iou = best_iou_for_image 
                        # --- END MODIFICATION ---
                ...
                all_outputs.append({
                    ...
                    "union": int(union),
                    "bbox_iou": bbox_iou,
                    "pred_bboxes": bboxes,               # Added predicted bboxes
                    "gt_bbox": batch_id_list[id_idx].get("bbox", []) # Added ground truth bbox
                })
