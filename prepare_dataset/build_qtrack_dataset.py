...
python build_qtrack_dataset.py \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert QTrack MOT dataset to HuggingFace MCP format")
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        help="Train directory inside dataset root"
    )
    parser.add_argument(
        "--jsonl",
        type=str,
        default="annotations.jsonl",
        help="JSONL file name inside train_dir"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="hf_dataset_qtrack",
...
def main():
    ...
    JSONL_PATH = TRAIN_DIR / args.jsonl
    ...
    for obj_id, entries in tqdm(grouped.items(), desc="Processing objects"):
