...
def union_two_dict(dict1: Dict[str, Any], dict2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in dict2.items():
        if key in dict1:
            ...
            if dict1[key] != value:
                # Values are different - raise error
                raise ValueError(f"{key} in meta_dict1 and meta_dict2 are not the same: {dict1[key]} != {value}")
            # If values are equal, we can safely keep one of them (skip)
        else:
            dict1[key] = value
