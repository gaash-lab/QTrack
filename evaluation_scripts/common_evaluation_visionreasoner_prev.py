        ...
        if isinstance(result, list):
            ...
            for item in result:
                if "frame" in item:
                    ...
                    if bbox is not None:
                        parsed.append({"frame": item["frame"], "bbox": bbox})
            return parsed
    except Exception:
        pass

    # Regex fallback (accept bbox or bbox_2d)
    pattern = r'\{.*?"frame":\s*(\d+).*?"(?:bbox|bbox_2d)":\s*\[([^\]]+)\].*?\}'
    matches = re.findall(pattern, text)

    parsed = []
    for frame, bbox_str in matches:
        bbox = [float(x.strip()) for x in bbox_str.split(",")]
        parsed.append({"frame": int(frame), "bbox": bbox})
...
def evaluate_single_object(item, dataset_root, model, processor, device):
    ...
    question = (
        ...
        + "First you need to check the inital bounidng box and its motion, then you need to predict the next bounding box and then your predicted bounding box should be having a little distance from the initial bounding box or if the person is not moving then the predicted box should be same, look at pixel level details of image and give correct xywh formatted bounding box in output"
...
# ---------------- Main ----------------
def evaluate_model(json_path, root, out_json, model_path):
    ...
    for item in tqdm(data):
        if detect_format(item) == "single":
            avg, res = evaluate_single_object(item, root, model, processor, device)
        else:
            avg, res = evaluate_multi_object(item, root, model, processor, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json", default="/home/2022bite008/Tawheed/Generate_Dataset/MOT_grounding_Dataset/test/annotations_real.json")
    parser.add_argument("--dataset_root", default="/home/2022bite008/Tawheed/Generate_Dataset/MOT_grounding_Dataset/test")
    ...
    parser.add_argument("--model_path", default="/home/2022bite008/Wasif/RMOT_Checkpoints/QTrack_real/global_step_7196/actor/huggingface")
    parser.add_argument("--out", default="RMOT_logs/visionreasoner_bbox_eval_QTRACK_real_one_7196.json")
    args = parser.parse_args()

    evaluate_model(args.json, args.dataset_root, args.out, args.model_path)
