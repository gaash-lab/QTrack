...
# SAM3 IMPORTS - Modified for interactive prediction
...
def parse_args():
    ...
    parser.add_argument("--segmentation_model_path", type=str, default="facebook/sam3")
    # REMOVED: sam_config argument (not needed for SAM3)
    ...
    parser.add_argument("--sam3_resolution", type=int, default=1008, 
                       help="SAM3 input resolution (default: 1008)")
...
def convert_bbox_to_cxcywh_norm(bbox, img_w, img_h):
    """Convert [x0, y0, x1, y1] bbox to normalized [cx, cy, w, h] for SAM3"""
    x0, y0, x1, y1 = bbox
    cx = (x0 + x1) / 2.0 / img_w
    cy = (y0 + y1) / 2.0 / img_h
    w = (x1 - x0) / img_w
...
def main():
    ...
    # Check dependencies
    print("[Eval] Checking dependencies...")
    try:
        import accelerate
        print(f"[Eval] Accelerate version: {accelerate.__version__}")
    except ImportError:
        print("[Warning] Accelerate not found. Will use manual device placement.")
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[Eval] Using device: {device}")
    if torch.cuda.is_available():
        print(f"[Eval] GPU: {torch.cuda.get_device_name(0)}")
        print(f"[Eval] CUDA version: {torch.version.cuda}")
    ...
    try:
        ...
        # Fallback: load without device_map and manually move to GPU
        print(f"[Warning] device_map='auto' failed, using manual device placement: {e}")
        reasoning_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            args.reasoning_model_path,
            torch_dtype=torch.bfloat16,
            attn_implementation="flash_attention_2",
    ...
    # === LOAD SAM3 MODEL (FIXED FOR PYTORCH 2.6 SECURITY ERROR) ===
    print(f"[Eval] Loading SAM3 from: {args.segmentation_model_path}")
    
    if os.path.exists(args.segmentation_model_path):
        print(f"[Eval] Local checkpoint detected. Using Robust Loader...")
        # 1. Initialize empty model (skipping the library's crashing loader)
        sam3_model = build_sam3_image_model(checkpoint_path=None)
        
        # 2. Manual Load with weights_only=False (Bypasses the "UnpicklingError")
        print("[Eval] Manually loading checkpoint to handle PyTorch 2.6+ security...")
        checkpoint = torch.load(args.segmentation_model_path, map_location="cpu", weights_only=False)
        
        # 3. Extract model state
        if "model" in checkpoint:
            state_dict = checkpoint["model"]
        else:
            state_dict = checkpoint
            
        # 4. Fix key prefixes (Training script added 'detector.', inference model doesn't need it)
        new_state_dict = {}
        ...
        missing, unexpected = sam3_model.load_state_dict(new_state_dict, strict=False)
        if len(missing) > 0:
            print(f"[Warning] Missing keys: {len(missing)}")
        print("[Eval] Local weights loaded successfully!")
    else:
        # Default HuggingFace Loading
        print(f"[Eval] Loading from HuggingFace: {args.segmentation_model_path}")
    ...
    print("[Eval] SAM3 loaded successfully")
    ...
    for i in tqdm(range(0, len(messages), args.batch_size)):
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            for id_idx in range(len(batch_output_text)):
                ...
                try:
                    img_h = batch_id_list[id_idx]["img_height"]
                    img_w = batch_id_list[id_idx]["img_width"]
                    
                    # Initialize accumulator at ORIGINAL image size
                    mask_all = np.zeros((img_h, img_w), dtype=bool)
                    
                    # Process each bbox+point pair INDIVIDUALLY
                    for bbox_idx, (bbox, point) in enumerate(zip(bboxes, points)):
                        # Create fresh state for each prediction
                        sam3_state = sam3_processor.set_image(batch_id_list[id_idx]["image"])
                        
                        # Convert bbox to SAM3 format [cx, cy, w, h] normalized
                        bbox_norm = convert_bbox_to_cxcywh_norm(bbox, img_w, img_h)
                        ...
                        # Get masks from SAM3
                        if "masks" in sam3_state and len(sam3_state["masks"]) > 0:
                            masks_tensor = sam3_state["masks"]  # Shape: [N, 1, H, W]
                            scores_tensor = sam3_state["scores"]
                            
                            # Get highest scoring mask
                            best_idx = scores_tensor.argmax().item()
                            # Extract: [1, H, W] -> [H, W]
                            mask = masks_tensor[best_idx, 0].cpu().numpy()
                            
                            # Convert to boolean (threshold at 0.5)
                            mask = (mask > 0.5).astype(bool)
                            
                            # CRITICAL: Verify and force resize if needed
                    ...
                    if batch_id_list[id_idx]["mask"] is not None:
                        gt_mask = np.array(batch_id_list[id_idx]["mask"]).astype(bool)
                        
                        # Ensure GT mask matches size
                        if gt_mask.shape != (img_h, img_w):
                            gt_mask = np.array(
                                PILImage.fromarray(gt_mask.astype(np.uint8) * 255).resize(
                                    (img_w, img_h),
                                    resample=PILImage.NEAREST
                                )
                            ).astype(bool)
                    ...
                    import traceback
                    print(f"SAM3 error for {batch_id_list[id_idx]['image_id']}: {e}")
                    if id_idx == 0:  # Print full traceback for first error only
                        traceback.print_exc()
                    pass
                ...
                all_outputs.append({
                    ...
                    "head_bbox_iou": float(head_bbox_iou), # Head IoU
                    "head_pred_bbox": head_bbox_px # Head Prediction
                })
