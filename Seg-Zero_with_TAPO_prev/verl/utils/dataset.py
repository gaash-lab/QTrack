...
def collate_fn(features: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    ############# TAPO Implementation ##################
    for key, value in tensors.items():
        # Don't stack pixel_values and image_grid_thw if they exist
...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # For Our specific MOT dataset format, we need custom prompt building
        row_dict = example.copy()
        
        # Build prompt for MOT task
        user_prompt = build_prompt(
            ...
            images=row_dict[self.image_key] if self.image_key in row_dict else []
        )

        messages = [
            {"role": "system", "content": self.system_prompt if hasattr(self, 'system_prompt') else ""},
            {"role": "user", "content": user_prompt},
        ]

        prompt = self.tokenizer.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)

        # Handle images like in old version
        if self.image_key in row_dict:
            ...
            processed_images = []
            for image in images:
                if isinstance(image, str) and self.image_dir is not None:
                    image_path = os.path.join(self.image_dir, image)
                    processed_images.append(process_image(image_path, self.min_pixels, self.max_pixels))
                else:
                    processed_images.append(process_image(image, self.min_pixels, self.max_pixels))
            
            # Get image inputs from processor
            ...
            # Expand image tokens in prompt like old version
        ...
        # Tokenize like old version
        input_ids, attention_mask = VF.tokenize_and_postprocess_data(
            prompt=prompt,
        ...
        # Get position ids
        if self.image_key in row_dict and image_grid_thw is not None:
            position_ids = get_rope_index(
                self.processor,
                input_ids=input_ids,
